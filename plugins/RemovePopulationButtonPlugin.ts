...
export class RemovePopulationButtonPlugin extends Plugin {
  init() {
    const editor = this.editor;
    editor.ui.componentFactory.add("removePopulation", (locale) => {
      const view = new ButtonView(locale);

      view.class = "removepopulationtoolbar";
      view.set({
      ...
      view.on("execute", () => {
        const now = new Date();
        let removePopulation = document.getElementById(
          "removePopulation_Editor"
        );
        if (removePopulation) {
...
export default RemovePopulationButtonPlugin;